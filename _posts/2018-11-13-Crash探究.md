...
下面，我将从操作系统层面来谈谈iOS中App的Crash，并结合实际开发工作，给出一些排查建议。
...
操作系统的异常分为4种。中断（interrupt），陷阱（trap），故障（fault）和中止（abort）。 
...
App的Crash是由操作系统的trap所触发的异常，在经过一系列流程之后，最终的表现形式。  
...
如下代码，在C语言中是不会Crash的，但是打印出来的内容是未知的。  
```
int main () {
    char *str = "bitnpc";
    char c = str[666];
...
其顺序为Mach内核->Unix->NSException。  
所以捕获的时候，就可以分别在对应的层次捕获堆栈信息。  
最后，把捕获到的CrashLog符号化，转化为可读的堆栈信息。  

### Mach异常 
Mach异常是最底层的内核级异常，如EXC_BAD_ACCESS。其他异常可见
[exception_types](https://opensource.apple.com/source/xnu/xnu-3789.1.32/osfmk/mach/exception_types.h.auto.html)。在异常发生时，会被异常处理程序转换为Mach消息，接着依次投递到thread，task和host端口。
...
```
Mach异常即使注册了对应的处理，也不会影响原先的传递流程。Mach异常会继续传递到Unix层，转变为Unix Signal。一个Mach异常对应一个或多个Unix Signal。
...
在Unix层则可以注册Signal处理回调。如下代码是把接受到的SIGSEGV和SIGINT信号统一用signalHander来处理。
```
...
......
49  imeituan                            -[TPKPOIDetailLookMoreItem initWithText:jumpUrlString:] (TPKPOIDetailLookMoreCell.m:60)
50  imeituan                            -[TPKPOIInfoPOIInformationViewModel itemsWithModel:] (TPKPOIInfoPOIInformationViewModel.m:102)
51  imeituan                            __51-[TPKPOIInfoPOIInformationViewModel setupViewModel]_block_invoke (TPKPOIInfoPOIInformationViewModel.m:43)
......
...
接下来，就看看能否复现。找到TPKPOIInfoPOIInformationViewModel所对应的页面，发现没有发生Crash。考虑到Crash的线程是后台线程，猜测很有可能是多线程创建TPKPOIDetailLookMoreItem导致的问题。那就可以写一些测试代码来尝试复现。注意，注意该段代码执行的时机要和实际创建该item一致。
```
for (int i = 0; i < 10; i++) {
    dispatch_async(dispatch_get_global_queue(DISPATCH_PRIORITY_DEFAULT, 0), ^{
        TPKPOIDetailLookMoreItem *item = [[TPKPOIDetailLookMoreItem alloc] initWithText:@"bit" jumpUrlString:@"npc"];
    });
}
```
...
1. 使用objc_registerClassPair方法，动态一个新类：NSKVONotifying_XXXXXX
...
```
到这里，原因已经很清晰了。  
项目中某基础库创建了两个TPKPOIDetailLookMoreItem_XXX，我们暂且称之为中间类。  
KVO用这两个中间类创建子类时，因为没有分配到内存空间，导致objc_registerClassPair时，发生了crash。  
解决方案则是在创建中间类时，对self.class加锁，保证只生成一个中间类。

# 总结
本文从操作系统层面中讨论了Crash的本质，成因，传递流程，说明了Crash的堆栈捕获层次与符号化方式，简要说明了Crash的排查思路，并给出了一个案例。  
