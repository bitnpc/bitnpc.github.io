...
Swift の async/await は**構造化並行性（Structured Concurrency）**モデルに基づいており、これがその中核的設計思想であり、2つの独立したメカニズムから構成される：

**① Continuation ステートマシン（async 関数のライフサイクル）**

```text
     実行 ── await ──→ サスペンド（スレッド制御を返す）
       ↑                       │
       │                       ▼
     再開 ←── 完了 ────── 非同期操作を待機
```

async 関数が `await` に到達するとサスペンドし、スレッドをプールに返して他のタスクが使用できるようにする。非同期操作が完了すると実行を再開。これは循環型のステートマシンであり、一度きりの順次フローではない。

**② Task 階層ツリー（構造化並行性の中核）**
...
親子 Task 間の重要な制約：
- **スコープのネスト**：子タスクは親のスコープを超えて生存できない（コンパイラ保証）
- **ライフサイクル管理**：親タスクはすべての子タスクの完了を待ってから終了
- **自動キャンセル**：親キャンセル → すべての子に再帰的に伝播

**中核概念：**

1. **Continuation（継続）**：async 関数が `await` ポイントでサスペンドされると、Swift は実行状態を保存する Continuation を作成し、後で再開して実行を継続する
2. **タスクツリー**：すべての非同期タスクはツリー構造を形成し、親子関係はスコープのネストによって関連付けられる。子タスクは親のスコープを超えて生存できない
3. **自動キャンセル伝播**：キャンセル信号は親から子へ再帰的に伝播し、手動管理は不要
### 1.2 コンパイラ変換メカニズム
