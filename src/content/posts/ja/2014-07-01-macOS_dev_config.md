...
tags:
    ...
    - macOS
toc: true
description: 'macOS 環境を初めてセットアップする、または再インストール後に構成する際、ツールチェインのバージョン、インストールパス、権限制御のあたりでつまずきやすいものです。本記事では「システム準備 → パッケージ管理 → ターミナルとシェル → 言語ランタイム → バージョン管理 → コンテナと一般ツール」の順に汎用的なフローを整理し、スムーズに開発を始められるようにします。'
---
