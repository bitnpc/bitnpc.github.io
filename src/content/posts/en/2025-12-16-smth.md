...
translationKey: smth
toc: true
description: 'This article explores building a multi-platform SMTH BBS client (iOS/iPadOS/macOS) with SwiftUI, detailing MVVM architecture, dependency injection, pagination state management, SwiftUI best practices, and cross-platform adaptation strategies.'
---
...
```text
  UI Layer              Business              Data Access              Network
┌──────────┐   ┌──────────────┐   ┌────────────────┐   ┌──────────────┐
...
Code push/PR
     │
     ▼
GitHub Actions triggered ──→ Setup Xcode 16.2 ──→ Install SwiftLint
     │
     ▼
Run Fastlane CI ──→ SwiftLint check ──→ (Passed?) ─┤
                                                     │ No
                                                     ▼
                                                 CI failed
                                                     │ Yes
