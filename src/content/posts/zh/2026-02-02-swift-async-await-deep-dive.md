...
Swift 的 async/await 基于**结构化并发**（Structured Concurrency）模型，这是其核心设计理念：

```mermaid
flowchart TD
    A[async 函数调用] --> B[创建 Continuation]
    B --> C[挂起当前任务]
    C --> D[保存执行上下文]
    D --> E[等待异步操作完成]
    E --> F[恢复执行]
...
1. **Continuation（延续）**：当 async 函数挂起时，Swift 会创建一个 continuation 来保存当前的执行状态
2. **任务树**：所有异步任务形成树状结构，父任务自动管理子任务的生命周期
3. **自动取消传播**：父任务取消时，所有子任务自动取消
