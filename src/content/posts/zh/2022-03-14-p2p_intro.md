...
tags:
    ...
    - 计算机网络
toc: true
description: '我们知道，常规的视频直播通常是通过 rtsp 或者 hls 协议直播的，这种直播的特点是所有的流量都需要从中心服务器发出，经过 CDN(Content Delivery Network)，下发到各个终端。'
---
...
![NAT](../../../assets/images/posts/post-2022-03-14/nat.jpg)_NAT示意图_
...
```alert
type: success
description: NAT 转换表的生成与 TCP/IP 报文的 IP 地址、端口号转换操作会产生一定的开销。
```
...
```alert
type: success
description: TCP 可以打洞吗？UDP 打洞和 TCP 打洞的本质是一样的，都是通过在 NAT 设备上创建映射来实现两个位于不同 NAT 后面的主机之间的直接通信。但是，由于 UDP 协议的特点，UDP 打洞更加简单和高效。
```
